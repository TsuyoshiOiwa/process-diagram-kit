---
name: bpmn-diagram
description: D2 による BPMN 業務プロセス図 (.d2) の作成・編集に使用。works/・examples/・templates/ 配下の .d2 におけるプール・レーン・タスク・イベント・ゲートウェイ・データ要素・フローの追加・修正、レイアウト調整、新規図の作成時にトリガーする。
---
...
```d2
承認: @../lib/components/task.task
承認: 承認処理
承認.desc.label: 人間が確認する処理   # 説明文 (任意)
```
...
## レイアウト規約 (examples/pizza-delivery.d2 準拠)

- 最上位は `direction: down` とし、プールは `top` / `left` で縦に積む (2つ目以降は `top` を大きく空ける)
- プール内の direction: レーンなし → `right` (横流れ)、レーンあり → `down`
- レーンは `top` / `left` を明示して段組みし、レーン内は `direction: right` (左→右に流す)
- 各レーンの先頭要素は `top: 0` / `left: 0` で原点に固定する
- 揃えたい終端イベントなどは明示座標で固定する (例: `食事.top: 0` `食事.left: 1700`)
...
- タイトルは md ブロック + `title.near: top-center` (更新日時を記載)
